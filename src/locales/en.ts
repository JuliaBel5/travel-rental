import type { Dictionary } from "./ru";

export const en: Dictionary = {
  common: {
    appName: "Wanderstay",
    tagline: "Stays for your travels",
    search: "Search",
    loading: "Loading…",
    description: "Wanderstay — book unique travel stays around the world.",
  },
  nav: {
    home: "Home",
    listings: "Stays",
    favorites: "Favorites",
    bookings: "My bookings",
    account: "Account",
    language: "Language",
    menu: "Menu",
    skipToContent: "Skip to content",
    toggleTheme: "Toggle theme",
  },
  auth: {
    logIn: "Log in",
    signUp: "Sign up",
    signOut: "Sign out",
    login: {
      title: "Log in",
      subtitle: "Sign in to book your stay.",
      email: "Email",
      password: "Password",
      submit: "Log in",
      submitting: "Signing in…",
      error: "Invalid email or password.",
      tooMany: "Too many sign-in attempts. Please try again later.",
      noAccount: "No account yet?",
      signUpLink: "Create one",
    },
    signup: {
      title: "Sign up",
      subtitle: "Create an account to book stays.",
      name: "Name",
      email: "Email",
      password: "Password",
      submit: "Create account",
      submitting: "Creating…",
      error: "Couldn't sign you up. Please try again.",
      emailTaken: "That email is already registered.",
      tooMany: "Too many attempts. Please try again later.",
      nameTooShort: "Enter a name (at least 2 characters)",
      emailInvalid: "Enter a valid email",
      passwordTooShort: "Password must be at least 8 characters",
      haveAccount: "Already have an account?",
      loginLink: "Log in",
    },
  },
  notFound: {
    title: "Page not found",
    subtitle: "The page you're looking for doesn't exist or has moved.",
    backHome: "Back to home",
    browseStays: "Browse stays",
  },
  home: {
    metaDescription:
      "Find the perfect place for your next trip — seaside villas, downtown lofts, mountain cabins and cozy countryside escapes.",
    hero: {
      eyebrow: "Over 1,000 hand-picked stays",
      title: "Your next journey starts here",
      subtitle:
        "Book one-of-a-kind homes around the world, from beachfront villas to alpine chalets.",
      imageAlt: "Tropical beach at sunset",
    },
    categories: {
      title: "Where to next?",
      subtitle: "Pick the mood of your trip",
    },
    featured: {
      title: "Featured stays",
      subtitle: "The highest-rated homes for your getaway",
      viewAll: "View all",
    },
  },
  search: {
    locationPlaceholder: "Where are you going?",
    addDates: "Dates",
    guests: {
      label: "Guests",
      hint: "How many are traveling",
      one: "guest",
      few: "guests",
      many: "guests",
    },
  },
  listing: {
    perNight: "night",
    reviews: {
      one: "review",
      few: "reviews",
      many: "reviews",
    },
  },
  catalog: {
    title: "Stays",
    filters: "Filters",
    sortLabel: "Sort",
    sort: {
      recommended: "Recommended",
      priceAsc: "Price: low to high",
      priceDesc: "Price: high to low",
      ratingDesc: "Top rated",
    },
    price: "Price per night",
    min: "Min",
    max: "Max",
    propertyType: "Property type",
    amenities: "Amenities",
    guests: "Guests",
    any: "Any",
    apply: "Apply",
    reset: "Reset",
    clearAll: "Clear all",
    stays: {
      one: "stay",
      few: "stays",
      many: "stays",
    },
    empty: {
      title: "No stays found",
      subtitle: "Try adjusting or clearing your filters.",
    },
    propertyTypes: {
      apartment: "Apartment",
      house: "House",
      villa: "Villa",
      room: "Room",
      cabin: "Cabin",
    },
  },
  detail: {
    reviews: "Reviews",
    amenities: "Amenities",
    whatThisPlaceOffers: "What this place offers",
    description: "About this place",
    whereYoullBe: "Where you'll be",
    hostedBy: "Meet your host",
    superhost: "Superhost",
    joinedIn: "Joined in",
    bedrooms: {
      one: "bedroom",
      few: "bedrooms",
      many: "bedrooms",
    },
    beds: {
      one: "bed",
      few: "beds",
      many: "beds",
    },
    bathrooms: {
      one: "bathroom",
      few: "bathrooms",
      many: "bathrooms",
    },
  },
  booking: {
    reviewTitle: "Confirm your booking",
    bookingAs: "Booking as",
    reserve: "Reserve",
    pickDates: "Add dates to see the total",
    cleaningFee: "Cleaning fee",
    serviceFee: "Service fee",
    total: "Total",
    nights: {
      one: "night",
      few: "nights",
      many: "nights",
    },
    youWontBeCharged: "You won't be charged yet",
    checkIn: "Check-in",
    checkOut: "Check-out",
    title: "Confirm your booking",
    summary: "Your trip",
    form: {
      heading: "Guest details",
      name: "Full name",
      namePlaceholder: "Your name",
      email: "Email",
      emailPlaceholder: "you@example.com",
      nameTooShort: "Please enter your full name (min. 2 characters)",
      emailInvalid: "Please enter a valid email address",
      submit: "Confirm booking",
      submitting: "Booking…",
      error: "We couldn't create your booking. Please try again.",
      unavailable: "Those dates are no longer available. Please pick another range.",
    },
    confirm: {
      title: "Booking confirmed!",
      subtitle: "Thank you! We've sent the details to your email.",
      bookingId: "Booking ID",
      viewMyBookings: "View my bookings",
      backHome: "Back to home",
      notFoundTitle: "Booking not found",
      notFoundSubtitle: "This link may have expired. Check your bookings instead.",
    },
    myBookings: {
      emptyTitle: "No bookings yet",
      emptySubtitle: "Find a stay and book your first trip.",
      browseStays: "Browse stays",
      cancel: "Cancel",
      confirmCancel: "Really cancel?",
      cancelling: "Cancelling…",
      cancelError: "Couldn't cancel. Please try again.",
    },
  },
  favorites: {
    add: "Save to favorites",
    remove: "Remove from favorites",
    emptyTitle: "No favorites yet",
    emptySubtitle: "Tap the heart on any listing card to save places you love.",
    browseStays: "Browse stays",
  },
  account: {
    title: "Account",
    subtitle: "Manage your profile and account settings.",
    profile: {
      title: "Profile",
      description: "Your name and photo, visible to others.",
      displayName: "Display name",
      displayNamePlaceholder: "Your name",
      nameTooShort: "Enter a name (at least 2 characters)",
      nameTooLong: "Name is too long (max 80 characters)",
      save: "Save",
      saving: "Saving…",
      saved: "Changes saved",
      saveError: "Couldn't save. Please try again.",
    },
    avatar: {
      upload: "Upload photo",
      change: "Change photo",
      remove: "Remove photo",
      uploading: "Uploading…",
      error: "Couldn't upload the photo. Try another image.",
      hint: "PNG, JPG or WebP. A square photo looks best.",
    },
    info: {
      title: "Account details",
      description: "Your core account information.",
      email: "Email",
      emailHint: "Email can't be changed.",
      memberSince: "Member since",
      bookings: "Bookings",
      favorites: "Favorites",
    },
    password: {
      title: "Change password",
      description: "Update your account password.",
      currentPassword: "Current password",
      newPassword: "New password",
      currentRequired: "Enter your current password",
      passwordTooShort: "Password must be at least 8 characters",
      submit: "Update password",
      submitting: "Updating…",
      success: "Password updated",
      wrongCurrent: "Current password is incorrect",
      tooMany: "Too many attempts. Please try again later.",
      error: "Couldn't change your password. Please try again.",
    },
    danger: {
      title: "Delete account",
      description: "Your account and related data will be permanently deleted.",
      delete: "Delete account",
      confirm: "Really delete your account?",
      confirmHint: "This can't be undone.",
      confirmDelete: "Yes, delete",
      cancel: "Cancel",
      deleting: "Deleting…",
      deleteError: "Couldn't delete your account. Please try again.",
    },
  },
  footer: {
    rights: "All rights reserved",
    madeWith: "Made for travelers",
  },
  locale: {
    ru: "Русский",
    en: "English",
  },
};
